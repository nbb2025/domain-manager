...
func (dq *DomainQuery) QueryUser() *UserQuery {
...
func (dq *DomainQuery) WithUser(opts ...func(*UserQuery)) *DomainQuery {
	query := (&UserClient{config: dq.config}).Query()
...
func (dq *DomainQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Domain, error) {
	...
	if query := dq.withUser; query != nil {
		if err := dq.loadUser(ctx, query, nodes, nil,
			func(n *Domain, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DomainQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Domain, init func(*Domain), assign func(*Domain, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Domain)
	for i := range nodes {
		if nodes[i].user_domains == nil {
			continue
		}
		fk := *nodes[i].user_domains
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_domains" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
